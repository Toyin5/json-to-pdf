...
import { Geometry, ProjectPages } from "../types/form-builder-type";
import {
...
const traverseArray = (
  ...
  for (const item of arr) {
    if (Array.isArray(item)) {
      ...
      result.push(...traverseArray(item, predicate));
    } else if (typeof item === "object" && item !== null) {
      // If the item is an object, recursively traverse its values
      result.push(...traverseObject(item, predicate));
    } else {
  ...
  for (const key in obj) {
    if (Object.hasOwnProperty.call(obj, key)) {
      ...
      if (Array.isArray(value)) {
        ...
        result.push(...traverseArray(value, predicate));
      } else if (typeof value === "object" && value !== null) {
        // If the value is an object, recursively traverse its values
        result.push(...traverseObject(value, predicate));
      } else {
...
export const exportAsPdf = async (
  ...
  const drawTxt = async ({
    ...
    const existingFont = fetchedFonts.find(
      (f) => f.fontName === fontName && f.style === `${fontWeight}-${fontStyle}`
    );
    ...
    if (fetchedImg) {
      ...
      if (objectFit !== "cover") {
        if (type.match(/png/i)) {
          ...
          canvas.width = img.naturalWidth * (type === "image/svg+xml" ? 2 : 1);
          canvas.height =
  ...
  for (const pageInfo of projectPages) {
    ...
    if (existingPage) {
      ...
      for (const k in FmbrPageSizeDimensionDefaultDetails) {
        const { width, height } = (FmbrPageSizeDimensionDefaultDetails as any)[
          k
        ][orientation];
        const eachWidth = +width.replace(/px/, "");
        const eachHeight = +height.replace(/px/, "");
        if (eachWidth / eachHeight === pdfPageSize.width / pdfPageSize.height) {
        ...
        for (const row of rowArray) {
          ...
          for (const cell of row) {
            ...
            if (showHorizontalBorder && i !== rowArray.length - 1) {
              ...
              const end = {
                x:
                  cellGeometryInfo.geometry.x + cellGeometryInfo.geometry.width,
                y:
                  cellGeometryInfo.geometry.y +
                  cellGeometryInfo.geometry.height,
              };
      ...
      if (control.value) {
        if (control.name.match(/signature|initials/i)) {
          ...
          const text = control.name.match(/date/i)
            ? DateTime.fromISO(control.value).toFormat(
                ctrlPropValue("Date format")
              )
            : control.name.match(/time/i)
            ? DateTime.fromISO(
                `${DateTime.now().toISODate()}T${control.value}`
              ).toFormat("hh:mm a")
            : control.value;
